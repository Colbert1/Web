...
<xmp>
  <php session_start(); define('LOGIN','Julien'); define('PASSWORD','1234'); $message='Bienvenue' ; if(!empty($_POST)) { if(empty($_POST['login'])) { $message='Indiquer votre login' ; } elseif(empty($_POST['motDePasse'])) { $message='Indiquer votre mot de passe' ; } elseif($_POST['login'] !==LOGIN) { $message='Login Erreur ' ; } elseif($_POST['motDePasse'] !==PASSWORD) { $message='Mot de passe non valide' ; } else { $message='Bienvenue ' . LOGIN .' !'; } } ?>
    <!DOCTYPE html>
    <html lang="en">
